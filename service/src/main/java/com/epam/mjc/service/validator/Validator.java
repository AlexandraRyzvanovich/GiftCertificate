...
public class Validator<T extends Identifiable> {
    ...
    public void validate(T item) throws ValidationException {
        if (GiftCertificate.class.equals(item.getClass())) { //instance of
            ...
            throw new ValidationException("Validator not specified for such class");
        }

    }
    private void validateCertificate(GiftCertificate giftCertificate) throws ValidationException {
        ...
        if(name == null || name.length() > NAME_MAX_LENGTH || name.isEmpty()) {
            throw new ValidationException("Gift certificate name is incorrect");
        }
        if(description == null || description.length() > DESCRIPTION_MAX_LENGTH || description.isEmpty()) {
            throw new ValidationException("Gift certificate description is incorrect");
        }
        if(price == null || price.compareTo(MIN_PRICE_VALUE) < 0 || price.compareTo(MAX_PRICE_VALUE) > 0 ) {
            throw new ValidationException("Gift certificate price is incorrect");
        }
        if(validDays == null || validDays < VALID_DAYS_MIN_VALUE || validDays > VALID_DAYS_MAX_VALUE) {
            throw new ValidationException("Gift certificate valid days field is incorrect");
        }
    }
    private void validateTag(Tag tag) throws ValidationException {
        String name =  tag.getName();
        if(name == null || name.length() > NAME_MAX_LENGTH || name.isEmpty()) {
            throw new ValidationException("Tag name is incorrect");
        }
    }
}
